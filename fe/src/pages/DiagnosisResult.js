...
const DiagnosisResult = () => {
    ...
    const loadData = async () => {
      try {
        ...
        if (diagnosisResult) {
          ...
          setError(
            "Không tìm thấy dữ liệu chẩn đoán. Vui lòng thực hiện chẩn đoán mới."
          );
        }
      } catch (err) {
  ...
  const handleSaveDiagnosis = async () => {
    ...
    try {
      ...
      if (result.symptoms && Array.isArray(result.symptoms)) {
        ...
        result.prediction_data?.symptoms &&
        Array.isArray(result.prediction_data.symptoms)
      ) {
      ...
      const response = await api.post("/api/ai/diagnoses/", {
        symptom_values:
          symptomArray.length > 0 ? symptomArray : [0, 0, 0, 0, 0, 0, 0, 0, 0],
  ...
  const diagnosis =
    result.disease_name ||
    result.prediction_data?.diagnosis ||
    result.diagnosis;
  const confidencePercent = Math.round(
    (result.prediction_data?.confidence || result.confidence) * 100
  );
  const uncertaintyPercent = Math.round(
    (result.prediction_data?.uncertainty || result.uncertainty) * 100
  );
  const testRecommendation =
    result.prediction_data?.test_recommendation || result.test_recommendation;
  const medicineRecommendation =
    result.prediction_data?.medicine_recommendation ||
    result.medicine_recommendation;

  // Xác định các triệu chứng đã chọn
  const presentSymptoms = (result.symptoms || []).filter(
    (symptom) => symptom.is_present
  );

  // Chuẩn bị dữ liệu biểu đồ cho Bar Chart
  const chartData = {
    probabilities:
      result.prediction_data?.probabilities || result.probabilities || {},
    uncertainties:
      result.prediction_data?.uncertainties || result.uncertainties || {},
  };

  // Chuẩn bị dữ liệu biểu đồ Radar
  const allSymptoms = result.symptoms?.map((s) => s.symptom_name) || [];
  const symptomValues =
    result.symptoms?.map((s) => (s.is_present ? 1 : 0)) || [];
    ...
    <Space direction="vertical" size="large" style={{ width: "100%" }}>
      ...
      >
        ...
        <Space>
          ...
          {user && (
            <Button icon={<HistoryOutlined />} onClick={goToHistory}>
      ...
      <Card>
        ...
        >
          ...
          {result.user_details && (
            ...
            >
              <UserOutlined /> {result.user_details.first_name}{" "}
              {result.user_details.last_name} ({result.user_details.username})
            </Text>
          )}
        </div>
        ...
        <div style={{ padding: "20px" }}>
          <Descriptions
            ...
            column={{ xs: 1, sm: 2 }}
          >
            <Descriptions.Item label="Bệnh">{diagnosis}</Descriptions.Item>
            ...
            <Col xs={24} md={12}>
              ...
              <div>
                {presentSymptoms.length > 0 ? (
                  ...
                  <Text type="secondary">
                    Không có triệu chứng nào được chọn.
                  </Text>
                )}
              </div>
            </Col>

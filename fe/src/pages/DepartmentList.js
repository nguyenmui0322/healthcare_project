...
const DepartmentList = () => {
  ...
  const columns = [
    ...
    { title: 'Mô tả', dataIndex: 'description', key: 'description' },
    {
      title: 'Số bệnh nhân',
      key: 'patients',
      render: (_, record) => Array.isArray(record.patients) ? record.patients.length : 0,
    },
    {
      title: 'Actions',
      key: 'actions',
  ...
  const expandable = {
    expandedRowRender: (record) => (
      <div>
        ...
        <ul>
          {Array.isArray(record.patients) && record.patients.length > 0 ? (
            record.patients.map(pid => <li key={pid}>Patient ID: {pid}</li>)
          ) : (
            <li>Không có bệnh nhân</li>
          )}
        </ul>
      </div>
    ),
    rowExpandable: record => Array.isArray(record.patients) && record.patients.length > 0,
  };

  return (
    <Card style={{ margin: 24, maxWidth: 1200, width: '100%', overflowX: 'auto' }}>
